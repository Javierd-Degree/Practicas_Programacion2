...
Status tree_preOrderRec(FILE* f, const Tree* pa, const NodeBT *n){
	Status s;
	if(f == NULL || pa == NULL) return ERROR;
	if(n == NULL) return OK;

	pa->print_element_function(f, INFO(n));
	s = tree_preOrderRec(f, pa, LEFT(n));
	if(s == ERROR) return ERROR;
	s = tree_preOrderRec(f, pa, RIGHT(n));
	if(s == ERROR) return ERROR;
	return OK;
}
...
Status tree_preOrder(FILE* f, const Tree* pa){
	if(f == NULL || pa == NULL) return ERROR;
	return tree_preOrderRec(f, pa, ROOT(pa));
}

Status tree_inOrderRec(FILE* f, const Tree* pa, const NodeBT *n){
	Status s;
	if(f == NULL || pa == NULL) return ERROR;
	if(n == NULL) return OK;

	s = tree_inOrderRec(f, pa, LEFT(n));
	if(s == ERROR) return ERROR;

	pa->print_element_function(f, INFO(n));

	s = tree_inOrderRec(f, pa, RIGHT(n));
	if(s == ERROR) return ERROR;
	return OK;
}
...
Status tree_inOrder(FILE* f, const Tree* pa){
	...
	s = tree_inOrderRec(f, pa, ROOT(pa));
	return s;
}


Status tree_postOrderRec(FILE* f, const Tree* pa, const NodeBT *n){
	Status s;
	if(f == NULL || pa == NULL) return ERROR;
	if(n == NULL) return OK;

	s = tree_postOrderRec(f, pa, LEFT(n));
	if(s == ERROR) return ERROR;
	s = tree_postOrderRec(f, pa, RIGHT(n));
	if(s == ERROR) return ERROR;
	pa->print_element_function(f, INFO(n));
	return OK;
}
